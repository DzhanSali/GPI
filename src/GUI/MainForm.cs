    ...
    public partial class MainForm : Form
    {
        ...
        private void toolStripButton5_Click(object sender, EventArgs e)
        {
            ...
            if (dialogProcessor.Selection.Count > 0)
            {
                ...
                statusBar.Items[0].Text = "Последно действие: Добавяне на групов примитив";
                viewPort.Invalidate();
            }
        }
        ...
        {
            FileStream fileStr = new FileStream("Serialized_Bin_Pic.bin", FileMode.Create);
            BinaryFormatter binary = new BinaryFormatter();
            binary.Serialize(fileStr, dialogProcessor.ShapeList);
            fileStr.Close();
            statusBar.Items[0].Text = "Последно действие: Запазване на бинарен файл";
        }

        private void toolStripButton7_Click(object sender, EventArgs e)
        {
            FileStream fileStr = new FileStream("Serialized_Bin_Pic.bin", FileMode.Open);
            BinaryFormatter binary = new BinaryFormatter();
            List<Shape> temp = (List<Shape>)binary.Deserialize(fileStr);
            dialogProcessor.ShapeList.Clear();
            dialogProcessor.ShapeList = temp;
            fileStr.Close();
            statusBar.Items[0].Text = "Последно действие: Отваряне на бинарен файл";
            viewPort.Invalidate();
        }
        ...
        private void toolStripButton8_Click(object sender, EventArgs e)
        {
            ...
            foreach (Shape s in dialogProcessor.Selection.ToList())
            {
                dialogProcessor.Selection.Remove(s);
                if (s.FillColor == Color.Red)
                {
        ...
        private void toolStripButton9_Click(object sender, EventArgs e)
        {
            ...
            {
                dialogProcessor.ShapeList.RemoveAt(i);
            }
            dialogProcessor.Selection.Clear();
            statusBar.Items[0].Text = "Последно действие: Изтриване на всички примитиви";
            viewPort.Invalidate();
        }
        ...
        private void toolStripMenuItem7_MouseEnter(object sender, EventArgs e)
        {
            ...
            {
                foreach (Shape s in dialogProcessor.Selection)
                {
                    s.BorderWidth = borderWidth;
                }
            }
            viewPort.Invalidate();
            statusBar.Items[0].Text = "Последно действие: Промяна на граничната дебелина";
        }
        ...
        private void toolStripButton10_Click(object sender, EventArgs e)
        {
            if (dialogProcessor.Selection != null)
            {
                const float rotationAngleIncrement = 90f;
                const int rotationCount = 3;

                for (int i = 0; i < rotationCount; i++)
                {
                    dialogProcessor.RotateSelection(rotationAngleIncrement);
                    viewPort.Invalidate();
                }
            }
            statusBar.Items[0].Text = "Последно действие: Промяна на граничната дебелина";
